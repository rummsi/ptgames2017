...
{block name="content"}        <center>
            ...
            <div style="top: 10px;" id="content">
                ...
                {ShowGalaxySelector($galaxy, $system)}
            {if $mode == 2}
                {ShowGalaxyMISelector($galaxy, $system, $planet, $CurrentPlanet['id'], $CurrentMIP)}
            {/if}
                <table width=569>
                    <tbody>
                        {ShowGalaxyTitles($galaxy, $system)}
                        {ShowGalaxyRows($galaxy, $system)}
                        {ShowGalaxyFooter($galaxy, $system, $CurrentMIP, $CurrentRC, $CurrentSP)}
                    </tbody>
                </table>
            </div>
        </center>{/block}