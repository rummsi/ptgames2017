	...
	if ($GalaxyRowPlayer['id'] != $user['id']) {
		...
		if ($GalaxyRowPlayer && $GalaxyRowPlanet["destruyed"] == 0) {
			...
			if ($user["settings_wri"] == "1" &&
				$GalaxyRowPlayer['id']) {
				$Result .= "<a href=game.php?page=messages&type=write&id=".$GalaxyRowPlayer["id"].">";
			...
			if ($user["settings_mis"] == "1" AND
				$MissileBtn == true          &&
				$GalaxyRowPlayer['id']) {
				$Result .= "<a href=game.php?page=galaxy&type=2&galaxy=".$Galaxy."&system=".$System."&planet=".$Planet."&current=".$user['current_planet']." >";
