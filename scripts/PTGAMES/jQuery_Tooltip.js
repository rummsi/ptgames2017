...
function initBuddyRequestForm() {
    $(".overlayDiv .buddyRequest form").unbind("submit").bind("submit", function () {
        var b = $(this);
        $.post(b.attr("action"), b.serialize()).done(function (a) {
            document.open();
            document.write(a);
            document.close()
        }).fail(function () {
...
ogame.chat = {
    ...
    initialize: function () {
        ...
        $("#chatMsgList").on("click", ".msg", function () {
            ...
            var a = $(".playerlist .playerlist_item[data-playerId=" + $(this).data("playerid") + "]").data("msgid");
            if (a) {
                b.loadChatLogWithPlayer(this, a)
            } else {
                b.loadChatLogWithPlayer(this)
            }
        });
        $(".chat").on("click", ".sys_msg", function (e) {
            var f = $(this).data("foreign-player-id");
            var a = {
                playerId: f,
                ajax: 1
            };
            $.ajax({
                ...
                data: a,
                success: function (c) {
                    $(".chat").each(function (h, d) {
                        if (f == $(d).data("foreign-player-id")) {
                            $(d).html(c)
                        }
                    })
                },
    ...
    updateTotalNewChatCounter: function () {
        var b = 0;
        if (ogame.messagecounter.newChats !== undefined) {
            for (key in ogame.messagecounter.newChats) {
                if (ogame.messagecounter.newChats.hasOwnProperty(key)) {
                    b += ogame.messagecounter.newChats[key]
                }
            }
        }
    ...
    sendMessage: function (r, m, l) {
        ...
        if (typeof l !== "undefined" && typeof l.id !== "undefined") {
            u.msg2reply = l.id
        }
        ...
        function p(a) {
            ...
            n.addChatItem(a.targetId, a.text, a.id, false, $refData, a.date)
        }
    ...
    messageReceived: function (d) {
        var f = ogame.chat;
        if (typeof d.refAuthor !== "undefined" && typeof d.refText !== "undefined") {
            $refData = {
                author: d.refAuhtor,
                text: d.refText
            }
        ...
        if ($(".chat_bar_list").length) {
            if (typeof f.data[d.senderId] == "undefined" && !(f.data[d.senderId])) {
                f.loadChatLogWithPlayer(d.senderId, null, function () {
                    f.addChatItem(d.senderId, d.text, d.id, true, $refData, d.date)
                }, false)
            } else {
                f.addChatItem(d.senderId, d.text, d.id, true, $refData, d.date)
            }
        }
        ogame.messagemarker.setPartnerId(d.senderId);
        if (!f.isOpen(d.senderId)) {
            ogame.messagecounter.initialize(ogame.messagecounter.type_chat, d.senderId);
            var e = parseInt(ogame.messagecounter.newChats[d.senderId]);
            if (isNaN(e)) {
                e = 0
            }
            e++;
    ...
    addChatItem: function (D, y, B, u, x, q) {
        var s = ogame.chat;
        var p = $(".chat_bar_list").find("[data-playerid='" + D + "']");
        var w = {};
        w.date = getFormatedDate(q, "[d].[m].[Y] <span>[H]:[i]:[s]</span>");
        w.newClass = "new";
        if (u) {
            w.playerName = s.data[D].playerName;
            w.altClass = ""
        } else {
        ...
        if (!p.length) {
            var C = s.createChatBarContainer(D);
            s.updateChatBar(C);
            p = $(".chat_bar_list").find("[data-playerid='" + D + "']")
        }
        var v = s.createChatItem(w);
        var r = s.getLastChatItemData();
        if (w.date != r.date || w.chatContent != r.text) {
            p.find(".chat").append(v);
            s.updateCustomScrollbar(p.find(".chat_box_ctn"));
            var A = $(".js_chatHistory");
            if (A.length && A.data("chatplayerid") == D) {
                A.find(".chat.clearfix").append(v.clone());
                s.updateCustomScrollbar($(".largeChatContainer"))
            }
        }
    },
    ...
    setVisibilityState: function () {
        var l = ogame.chat;
        var m = l.getVisibleChatPlayerIds();
        var h = $("#chatBar .chat_bar_list .chat_bar_list_item");
        for (var p = 0; p < h.length; p++) {
            var n = h.get(p);
            var k = $(n).data("playerid");
            if (!l.isInJson(k, m)) {
                l.closeChatBox(k)
            } else {
                n.style.display = "inline";
                if ($(n).hasClass("open")) {
                    var o = $(n).find("div.chat_box")[0];
                    o.style.display = "inline";
                    l.updateCustomScrollbar($(n).find(".chat_box_ctn"), 1)
                }
            }
        }
    },
    ...
    createChatItem: function (m) {
        ...
        g.append('<span class="msg_date fright">' + m.date + "</span>");
    ...
    $(".bday_box").click(function (d) {
        ...
        if (eventBoxData[a] !== undefined) {
            ...
            if (eventBoxData[a].eventImgSrc === undefined && eventBoxData[a].eventTypeClass === "future") {
                $("#eventBox" + a + " #BDayEventImg").attr("src", "//gf3.geo.gfsrv.net/cdn82/d995359d038c9a0c21aed16b3cc162.png")
            } else {
...
function refreshFormData() {
    ...
    styleClass = ((consumption > tankSpace) || (consumption > $("#resources_deuterium").text().replace(/\./g, ""))) ? "overmark" : "undermark";
...
function checkOk() {
    switch (currentPage) {
        case "fleet2":
            if ((currentGalaxy == $("#galaxy").val() && currentSystem == $("#system").val() && currentPosition == $("#position").val() && currentPlanetType == $("#type").val()) || $("#galaxy").val() == "" || $("#system").val() == "" || $("#position").val() == "" || consumption > currentDeuterium || consumption > tankSpace || cargoSpace < 0) {
                validated = false;
                $("#continue").attr("class", "off")
            } else {
                validated = true;
...
function createSummaryHtml(n) {
    ...
    var r = 0;
    var k = "";
    q = q + '<div class="planetHead"><div class="planetname">' + n.translations.summary + '</div><div class="planetImg"><img src="//gf3.geo.gfsrv.net/cdn8a/7efb2e73ca11d2344bbed43668da10.jpg"/></div><div class="planetData"><ul><li class="coords textLeft"></li><li class="fields textRight"></li></ul></div><div class="clearfloat"/></div>';
...
function createHeaderHtml(g) {
    ...
    h = '<div id="empireTab"><div class="wrapTab"><div class="tab-part01"></div><h2>' + g.translations.header + '</h2><span class="reset"><img src="//gf2.geo.gfsrv.net/cdnab/f805c477d15ae3131b7c39c7d70e48.gif" width="16" height="16"><a href="javascript:void(0);" onClick="clearImperiumOrder(); return false;">' + g.translations.reset + '</a></span><div class="wrapCorner"></div><br class="clearfloat"/></div></div><div class="header">' + h + "</div>";
    return h
}
...
ogame.messages.combatreport = {
    ...
    setShipCount4All: function (u) {
        ...
        for (var y in v) {
            var F = v[y];
            var s = (w.check(true, A, {
                types: {
                    0: "isEmpty"
                }
            }) && w.check(true, A[y])) ? A[y] : {};
    ...
    setShipCountByActiveMember: function (n, p, s) {
        ...
        for (var u in p) {
            ...
            if (m == s) {
                ...
                if (o.check(true, n.losses)) {
                    for (var q in n.losses[u]) {
                        if (!(w.hasOwnProperty(q))) {
                            w[q] = parseInt(n.losses[u][q])
                        } else {
                            w[q] = w[q] + parseInt(n.losses[u][q])
                        }
                    }
                }
            }
    ...
    setShipCount: function (g) {
        ...
        var h = g.losses;
        for (var f in e) {
            $(f).text(e[f].toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."))
        }
        for (var f in h) {
            $(f).text("-" + h[f].toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."))
        }
    },
    ...
    displayShipData: function (g, h, k, f) {
        ...
        if (l.check(true, g, {
            ...
            l.setShipCountByActiveMember(k, f, g)
        } else {
            l.setShipCount4All(l.data.combatRounds[h][l.data.combatRounds[h].length - 1])
        }
    },
    ...
    setCombatArray: function (e, f) {
        ...
        if (d.data.memberSelection) {
            ...
            d.data.activeMember = $("#" + d.data.combatside + "_select_combatreport").data("memberName")
        }
...
function closeDetails(f, d) {
    var e = $("#fleet" + f);
    e.children(".openDetails").children().children().attr("src", "//gf2.geo.gfsrv.net/cdn10/de1e5f629d9e47d283488eee0c0ede.gif");
...
function openDetails(f, d) {
    var e = $("#fleet" + f);
    e.children(".openDetails").children().children().attr("src", "//gf3.geo.gfsrv.net/cdnb6/577565fadab7780b0997a76d0dca9b.gif");
...
function openOverlay(N, G) {
    ...
    delete G.type;
    if (Q != "inline") {
        var P = $(document.createElement("img")).attr("src", "//gf1.geo.gfsrv.net/cdnc6/4161a64a933a5345d00cb9fdaa25c7.gif").attr("alt", LocalizationStrings.loading);
    ...
    var f = {
        ...
        selectPlanetOrMoon: function (b) {
            ...
            if ($(b).hasClass("js_honor")) {
                ...
                var p = '<img height="18" src="//gf1.geo.gfsrv.net/cdnfc/f35675179214f8f6f0f8d75740d7db.png" alt="' + loca.honorPoints + '"/><span class="option_source">' + loca.honorPoints + "</span>";
