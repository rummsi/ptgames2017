...
if (!empty($post['suma'])) {
    if (!empty($post['edf']))
        setcoookie('edf', serialize($post['edf']), time() + 60 * 60 * 24 * 30);
    if (!empty($post['inv']))
        setcoookie('inv', serialize($post['inv']), time() + 60 * 60 * 24 * 30);
    if (!empty($post['han']))
        setcoookie('han', serialize($post['han']), time() + 60 * 60 * 24 * 30);
    if (!empty($post['def']))
        setcoookie('def', serialize($post['def']), time() + 60 * 60 * 24 * 30);
...
function LoadCookies() {
    ...
    if (!empty($_COOKIE['edf']))
        $edff = unserialize($_COOKIE['edf']);
    if (!empty($_COOKIE['inv']))
        $invv = unserialize($_COOKIE['inv']);
    if (!empty($_COOKIE['han']))
        $hann = unserialize($_COOKIE['han']);
    if (!empty($_COOKIE['def']))
        $deff = unserialize($_COOKIE['def']);
}
