...
if ($post["s"] == 1 || $post["s"] == 2) {//Edicion y agregar notas
    $time = time();
    $priority = $post["u"];
    $title = ($post["title"]) ? mysql_real_escape_string(strip_tags($post["title"])) : $lang['NoTitle'];
    $text = ($post["text"]) ? mysql_real_escape_string(strip_tags($post["text"])) : $lang['NoText'];
