...
document.createElement("canvas").getContext || function () {
    ...
    x.clearRect = function () {
        ...
        var I = this.getCoords_(G, H),
                J = this.getCoords_(L, D),
                K = this.getCoords_(E, F);
        y(this, J, K, I)
    }, x.quadraticCurveTo = function (F, G, H, I) {
        var J = this.getCoords_(F, G),
                D = this.getCoords_(H, I),
                E = {
        ...
        var N = M ? "at" : "wa",
                ...
                G = D + ae(K) * E - b;
        O == Q && !M && (O += 0.125);
        var H = this.getCoords_(L, D),
                I = this.getCoords_(O, P),
                J = this.getCoords_(Q, G);
        this.currentPath_.push({
            type: N,
            x: H.x,
            y: H.y,
        ...
        var I, K, L, M, N, O, P, Q, R = G.runtimeStyle.width,
                T = G.runtimeStyle.height;
        ...
        if (this.m_[0][0] != 1 || this.m_[0][1]) {
            ...
            var E = H,
                    S = this.getCoords_(I + L, K),
                    V = this.getCoords_(I, K + M),
                    W = this.getCoords_(I + L, K + M);
        ...
        var M = [],
                O = u(K ? this.fillStyle : this.strokeStyle),
                Q = O.color,
                R = O.alpha * this.globalAlpha;
        ...
        if (K) {
            if (typeof this.fillStyle == "object") {
                ...
                N = 0,
                        S = 1;
                if (H.type_ == "gradient") {
                    var V = H.x1_ / this.arcScaleX_,
                            X = H.y1_ / this.arcScaleY_,
                            Z = this.getCoords_(H.x0_ / this.arcScaleX_, H.y0_ / this.arcScaleY_),
                            ak = this.getCoords_(V, X);
                    J = Math.atan2(ak.x - Z.x, ak.y - Z.y) * 180 / Math.PI, J < 0 && (J += 360), J < 0.000001 && (J = 0)
                } else {
                    var Z = this.getCoords_(H.x0_, H.y0_),
                            I = D.x - U.x,
                            al = D.y - U.y;
                    L = {
                        x: (Z.x - U.x) / I,
                ...
                var T = P.length,
                        W = P[0].color,
                        Y = P[T - 1].color,
                        aa = P[0].alpha * this.globalAlpha,
                        ab = P[T - 1].alpha * this.globalAlpha,
                        ad = [],
                        E = 0;
                for (; E < T; E++) {
                    var G = P[E];
        ...
        this.arcScaleX_ *= D, this.arcScaleY_ *= E, l(this, r([
            [D, 0, 0],
            [0, E, 0],
            [0, 0, 1]
        ], this.m_), !0)
