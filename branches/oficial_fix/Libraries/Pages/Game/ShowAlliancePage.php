                ...
                if ($_POST) { // esta parte es igual que el buscador de search.php...
                    ...
                    if (mysqli_num_rows($search) != 0) {
                        ...
                        while ($s = mysqli_fetch_array($search)) {
                            $entry = array();
                            $entry['ally_tag'] = "[<a href=\"game.php?page=alliance&type=apply&allyid={$s['id']}\">{$s['ally_tag']}</a>]";
        ...
        elseif ($user['ally_id'] != 0 && $user['ally_request'] == 0) { // Con alianza
            ...
            if ($mode == 'exit') {
                ...
                if ($_GET['yes'] == 1) {
                    ...
                    $page = MessageForm($lang['Want_go_out'], "<br>", "game.php?page=alliance&type=exit&yes=1", "Oui");
                }
                display($page);
            }
                ...
                if (count($ally_ranks) == 0 || $ally_ranks == '') { // si no hay rangos
                    ...
                    foreach ($ally_ranks as $a => $b) {
                        if ($ally['ally_owner'] == $user['id']) {
                            // $i++;u2r5
                            $lang['id'] = $a;
                            $lang['delete'] = "<a href=\"game.php?page=alliance&type=admin&edit=rights&d={$a}\"><img src=\"{$dpath}pic/abort.gif\" alt=\"{$lang['Delete_range']}\" border=0></a>";
                            $lang['r0'] = $b['name'];
                            $lang['a'] = $a;
			...
			WHERE `id`='{$ally['id']}'", "alliance");
                ...
                $lang['Transfer_alliance'] = MessageForm("Abandonner / Transf&eacute;rer L'alliance", "", "game.php?page=alliance&type=admin&edit=give", $lang['Continue']);
                $lang['Disolve_alliance'] = MessageForm("Dissoudre L'alliance", "", "game.php?page=alliance&type=admin&edit=exit", $lang['Continue']);
            ...
            if ($mode == 'admin' && $edit == 'exit') { // disolver una alianza
                ...
                if ($ally['ally_owner'] == $user['id'] || $ally_ranks[$user['ally_rank_id'] - 1]['memberlist'] != 0) {
                    $lang['members_list'] = " (<a href=\"game.php?page=alliance&type=memberslist\">{$lang['Members_list']}</a>)";
                } else {
                    $lang['members_list'] = '';
                }
                // El link de adminstrar la allianza
                if ($ally['ally_owner'] == $user['id'] || $ally_ranks[$user['ally_rank_id'] - 1]['administrieren'] != 0) {
                    $lang['alliance_admin'] = " (<a href=\"game.php?page=alliance&type=admin&edit=ally\">{$lang['Alliance_admin']}</a>)";
                } else {
                    $lang['alliance_admin'] = '';
                }
                // El link de enviar correo circular
                if ($ally['ally_owner'] == $user['id'] || $ally_ranks[$user['ally_rank_id'] - 1]['mails'] != 0) {
                    $lang['send_circular_mail'] = "<tr><th>{$lang['Circular_message']}</th><th><a href=\"game.php?page=alliance&type=circular\">{$lang['Send_circular_mail']}</a></th></tr>";
                } else {
                ...
                if ($request_count != 0) {
                    if ($ally['ally_owner'] == $user['id'] || $ally_ranks[$user['ally_rank_id'] - 1]['bewerbungen'] != 0)
                        $lang['requests'] = "<tr><th>{$lang['Requests']}</th><th><a href=\"game.php?page=alliance&type=admin&edit=requests\">{$request_count} {$lang['XRequests']}</a></th></tr>";
                }
                if ($ally['ally_owner'] != $user['id']) {
                    $lang['ally_owner'] .= MessageForm($lang['Exit_of_this_alliance'], "", "game.php?page=alliance&type=exit", $lang['Continue']);
                } else {
                    $lang['ally_owner'] .= '';
                }
